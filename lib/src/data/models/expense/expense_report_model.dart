...
class ExpenseReportModel extends ExpenseReportEntity{
  ...
  factory ExpenseReportModel.fromJson(Map<dynamic, dynamic> json) {

    Map<dynamic, dynamic>? response = json['expenseReport'];
    ...
    keys.forEach((element) {
      ...
      dateList.add(Date(
          ...
          filterList: response?[element]['accounts'] != null && (response?[element]['total'] is int ? double.parse('${response?[element]['total']}') : (response?[element]['total'] ?? 0))!=0 ? List<Filter>.from(response?[element]['accounts']!.map((x) => Filter.fromJson(x,element))) : [],
          reportDate: element
      ));
    });
