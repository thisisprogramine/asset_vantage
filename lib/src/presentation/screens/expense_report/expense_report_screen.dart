...
import 'package:asset_vantage/src/presentation/screens/expense_report/expense_universal_filter.dart';
...
class ExpenseReportScreen extends StatefulWidget {
  final ExpenseReportArgument argument;
  const ExpenseReportScreen({super.key, required this.argument});
...
class _ExpenseReportScreenState extends State<ExpenseReportScreen> {
  ...
  void selectChartBar(int index, {bool withoutSetState = false}) {
  ...
  List<Child>? getPositionData({required List<ExpenseChartData> chartData}) {
    if (currentLevel == 0) {
  ...
  Widget uiElementsToShowAfterDataLoad(
      Timer? timer, bool stealthValue, ExpenseReportLoaded state) {
    return BlocBuilder<ExpenseNumberOfPeriodCubit, ExpenseNumberOfPeriodState>(
        bloc: widget.argument.expenseNumberOfPeriodCubit,
      builder: (context, numberOfPeriodState) {
          if(state.chartData.isNotEmpty) {
            List<ExpenseChartData> expenseChartData = state.chartData;
            // if (currentLevel == 0) {
            //   expenseChartData = state.chartData.sublist(
            ...
            //
            // } else {
            //       expenseChartData = (state.chartData
            //       .map(
            //         (e) {
            //       final item = e.children.where(
            //             (element) {
            ...
            //     },
            //   )
            //       .toList() as List<ExpenseChartData>)
            //       .sublist(
            //           state.chartData.length -
            //           (numberOfPeriodState
            //               .selectedExpenseNumberOfPeriod
            ...
            final List<Child> positions = getPositionData(chartData: expenseChartData) ?? [];
            return Column(
              children: [
                ...
                Card(
                  ...
                  child: ExpenseHighlightWidget(
                    ...
                    expenseReportCubit: widget.argument.expenseReportCubit,
                    ...
                    expenseSortCubit: widget.argument.expenseSortCubit,
                    expenseLoadingCubit: widget.argument.expenseLoadingCubit,
                    expenseEntityCubit: widget.argument.expenseEntityCubit,
                    expenseAsOnDateCubit: widget.argument.expenseAsOnDateCubit,
                    expenseAccountCubit: widget.argument.expenseAccountCubit,
                    expensePeriodCubit: widget.argument.expensePeriodCubit,
                    expenseNumberOfPeriodCubit:
                    widget.argument.expenseNumberOfPeriodCubit,
                    expenseChartCubit: widget.argument.expenseChartCubit,
                    expenseCurrencyCubit: widget.argument.expenseCurrencyCubit,
                    expenseDenominationCubit: widget.argument.expenseDenominationCubit,
                ...
                if(positions.isNotEmpty)
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        BlocBuilder<ExpenseSortCubit, Sort>(
                            bloc: widget.argument.expenseSortCubit,
                            builder: (context, sortState) {
                              return SortFilter(
                                  onSelect: (int index) async {
                                    if (index == 1) {
                                      widget.argument.expenseSortCubit
                ...
                BlocBuilder<ExpenseChartCubit, int>(
                    bloc: widget.argument.expenseChartCubit,
                    builder: (context, expenseChartState) {
                      if (selectedChartBarMonth[currentLevel] <
                          (numberOfPeriodState
                              .selectedExpenseNumberOfPeriod?.value ??
                              0)) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: BlocBuilder<ExpenseReportCubit, ExpenseReportState>(
            bloc: widget.argument.expenseReportCubit,
            builder: (context, state) {
              ...
              } else if (state is ExpenseReportError) {
                if (state.errorType ==
  ...
  Widget _getDrilldownList(
    ...
    return BlocBuilder<ExpenseSortCubit, Sort>(
        bloc: widget.argument.expenseSortCubit,
        builder: (context, sortState) {
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final greaterValue = AppHelpers.getExpenseChildrenGreaterValue(
                  expenseList: positions ?? []);
...
class _ChildItemWithTooltipState extends State<ChildItemWithTooltip> {
  ...
  Widget build(BuildContext context) {
    Timer? timer;
    return BlocBuilder<ExpenseDenominationCubit, ExpenseDenominationState>(
        bloc: widget.argument.expenseDenominationCubit,
                      ...
                      children: [
                        Flexible(
                          child: AnimatedFractionallySizedBox(
                            ...
                            widthFactor: AppHelpers.getExpenseChartFraction(
                                actualValue: widget.item?.total ?? 0.0,
                        ...
                        Text(
                          "${NumberFormat.simpleCurrency(name: widget.argument.expenseCurrencyCubit.state.selectedExpenseCurrency?.code ?? 'USD').currencySymbol} ${"${(widget.item?.total ?? 0.0) < 0.0 ? '(${NumberFormat(widget.argument.expenseCurrencyCubit.state.selectedExpenseCurrency?.format ?? '###,###,##0.00').format((widget.item?.total ?? 0.0).abs().denominate(denominator: denominationState.selectedExpenseDenomination?.denomination))})' : NumberFormat(widget.argument.expenseCurrencyCubit.state.selectedExpenseCurrency?.format ?? '###,###,##0.00').format((widget.item?.total ?? 0.0).denominate(denominator: denominationState.selectedExpenseDenomination?.denomination))}${denominationState.selectedExpenseDenomination?.suffix}".stealth(widget.stealthValue)}",
                          style:
                                ...
                                children: [
                                  Flexible(
                                    child: AnimatedFractionallySizedBox(
                                      ...
                                      alignment: Alignment.centerLeft,
                                      widthFactor:
                                          AppHelpers.getExpenseChartFraction(
                                              actualValue:
