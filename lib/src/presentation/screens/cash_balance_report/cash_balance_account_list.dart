...
class CashBalanceAccountFilter extends StatefulWidget {
  final void Function() onDone;
  final ScrollController scrollController;
  final CashBalanceAccountCubit cashBalanceAccountCubit;

  const CashBalanceAccountFilter({
    super.key,
  ...
  @override
  State<CashBalanceAccountFilter> createState() => _CashBalanceAccountFilterState();
}

class _CashBalanceAccountFilterState extends State<CashBalanceAccountFilter> {
  List<SubGroupingItemData?> selectedAccounts = [];
  final TextEditingController controller = TextEditingController();

  @override
  void initState() {
    super.initState();
    selectedAccounts.addAll(widget.cashBalanceAccountCubit.state.selectedAccountsList ?? []);
  }
          ...
          children: [
            ...
            Expanded(
              child: RawScrollbar(
                ...
                child: ListView.builder(
                    ...
                    itemCount: widget.cashBalanceAccountCubit.state
                        .accountList?.length ??
                        0,
                    itemBuilder: (context, index) {
                      final filterData = widget.cashBalanceAccountCubit
                          ...
                          children: [
                            if (index == 0)
                              GestureDetector(
                                onTap: () {
                                  setState(() {
                                    if(selectedAccounts.length != (widget.cashBalanceAccountCubit.state.accountList?.length ?? 0)) {
                                      selectedAccounts.clear();
                                      selectedAccounts.addAll(widget.cashBalanceAccountCubit.state.accountList ?? []);
                                    }else {
                                      selectedAccounts.clear();
                                    }
                                  });
            ...
            Padding(
              ...
              child: ApplyButton(
                ...
                onPressed: () {
                  widget.cashBalanceAccountCubit.changeSelectedCashBalanceAccounts(selectedAccounts: selectedAccounts);
                  widget.onDone();
                },
              ),
            ),
