...
class PerformancePrimarySubGroupingFilter extends StatefulWidget {
  ...
  const PerformancePrimarySubGroupingFilter({
    super.key,
  ...
  @override
  State<PerformancePrimarySubGroupingFilter> createState() => _PerformancePrimarySubGroupingFilterState();
}

class _PerformancePrimarySubGroupingFilterState extends State<PerformancePrimarySubGroupingFilter> {
  ...
  void initState() {
    super.initState();
    selectedPrimarySubGrouping.addAll(widget.performancePrimarySubGroupingCubit.state.selectedSubGroupingList ?? []);
  }
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: FilterNameWithCross(text: StringConstants.primarySubGrpFilterString,
              isSubHeader: true,
              onDone: (){
                widget.onDone();
              },)
            ...
            Expanded(
              child: RawScrollbar(
                ...
                child: ListView.builder(
                    ...
                    itemCount: widget.performancePrimarySubGroupingCubit.state
                        .subGroupingList?.length ??
                        0,
                    itemBuilder: (context, index) {
                      final filterData = widget.performancePrimarySubGroupingCubit
            ...
            Padding(
              ...
              child: ApplyButton(
                ...
                onPressed: () async{
                  await widget.performancePrimarySubGroupingCubit.changeSelectedPerformancePrimarySubGrouping(selectedSubGroupingList: selectedPrimarySubGrouping);
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(widget.performancePrimaryGroupingCubit.state.selectedGrouping?.name?.replaceAll('-', ' ') ?? '--',
                style: Theme.of(context).textTheme.headlineMedium,
              ),
              GestureDetector(
                onTap: () {
                  widget.performancePrimarySubGroupingCubit.changeSelectedPerformancePrimarySubGrouping(selectedSubGroupingList: selectedPrimarySubGrouping);
                  widget.onDone();
                },
                child: Text('Apply',
                  style: Theme.of(context).textTheme.titleLarge?.copyWith(color: context.read<AppThemeCubit>().state?.bottomSheet?.backArrowColor != null ? Color(int.parse(context.read<AppThemeCubit>().state!.bottomSheet!.backArrowColor!)) : AppColor.primary, fontWeight: FontWeight.bold),
          ...
          child: StatefulBuilder(
              builder: (context, setState) {
                return ListView.builder(
                  controller: widget.scrollController,
                  physics: const BouncingScrollPhysics(),
                  itemCount: widget.performancePrimarySubGroupingCubit.state.subGroupingList?.length ?? 0,
                  itemBuilder: (context, index) {
                    final filterData = widget.performancePrimarySubGroupingCubit.state.subGroupingList?[index];
                    return Column(
                      children: [
                        if(index == 0)
                          GestureDetector(
                            onTap: () {
                              setState(() {
                                if(selectedPrimarySubGrouping.length != (widget.performancePrimarySubGroupingCubit.state.subGroupingList?.length ?? 0)) {
                                  selectedPrimarySubGrouping.clear();
                                  selectedPrimarySubGrouping.addAll(widget.performancePrimarySubGroupingCubit.state.subGroupingList ?? []);
                                }else {
                                  selectedPrimarySubGrouping.clear();
                                }
                              });
                        ...
                        GestureDetector(
                          onTap: () {
                            if(filterData != null) {
                              setState((){
                                if(selectedPrimarySubGrouping.contains(filterData)) {
                                  selectedPrimarySubGrouping.remove(filterData);
                                }else {
                                  selectedPrimarySubGrouping.add(filterData);
                                }
                              });
