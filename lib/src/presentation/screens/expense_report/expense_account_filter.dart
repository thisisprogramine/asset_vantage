...
class ExpenseAccountFilter extends StatefulWidget {
  final void Function() onDone;
  final ScrollController scrollController;
  final ExpenseAccountCubit expenseAccountCubit;

  const ExpenseAccountFilter({
    super.key,
  ...
  @override
  State<ExpenseAccountFilter> createState() => _ExpenseAccountFilterState();
}

class _ExpenseAccountFilterState extends State<ExpenseAccountFilter> {
  List<AccountEntity?> selectedAccounts = [];
  final TextEditingController controller = TextEditingController();

  @override
  void initState() {
    super.initState();
    selectedAccounts.addAll(widget.expenseAccountCubit.state.selectedAccountList ?? []);
  }
          ...
          children: [
            ...
            Expanded(
              child: RawScrollbar(
                ...
                child: ListView.builder(
                    ...
                    itemCount: widget.expenseAccountCubit.state
                        .accountList?.length ??
                        0,
                    itemBuilder: (context, index) {
                      final filterData = widget.expenseAccountCubit
                          ...
                          children: [
                            if (index == 0)
                              GestureDetector(
                                onTap: () {
                                  setState(() {
                                    if(selectedAccounts.length != (widget.expenseAccountCubit.state.accountList?.length ?? 0)) {
                                      selectedAccounts.clear();
                                      selectedAccounts.addAll(widget.expenseAccountCubit.state.accountList ?? []);
                                    }else {
                                      selectedAccounts.clear();
                                    }
                                  });
            ...
            Padding(
              ...
              child: ApplyButton(
                ...
                onPressed: () {
                  widget.expenseAccountCubit.changeSelectedExpenseAccount(selectedExpenseAccountList: selectedAccounts);
                  widget.onDone();
                },
              ),
            ),
